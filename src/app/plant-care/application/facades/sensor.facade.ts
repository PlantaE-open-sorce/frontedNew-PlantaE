...
export class SensorFacade {
  ...
  linkSensor(
    sensorId: string,
    plantId: string,
    context?: { sensorLabel?: string; plantLabel?: string }
  ) {
    this.repository.link(sensorId, { plantId }).subscribe({
      next: () => {
        const time = new Date().toLocaleString();
        const sensorText = context?.sensorLabel ?? sensorId;
        const plantText = context?.plantLabel ?? plantId;
        this.notifications.success(`Sensor ${sensorText} vinculado a planta ${plantText} · ${time}`);
        this.searchSensors();
      },
